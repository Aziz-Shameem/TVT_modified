...
from models.modeling import VisionTransformer, CONFIGS, AdversarialNetwork, WaveFit
...
def setup(args):
    # Prepare model
    if args.model_type=="wavemix" :
        config = CONFIGS[args.model_type]
        model = WaveFit(config, args.img_size, args.num_classes, zero_head = True)
...
def train(args, model):
    ...
    if args.model_type=="wavemix" :
        optimizer = torch.optim.SGD([
                                    {'params': model.parameters(), 'lr': args.learning_rate/10},
                                ],
...
def main():
    ...
    parser.add_argument("--model_type", choices=["ViT-B_16", "ViT-B_32", "ViT-L_16",
                                                 "ViT-L_32", "ViT-H_14", "R50-ViT-B_16", "wavemix"],
                        default="ViT-B_16",
                        help="Which variant to use.")
    parser.add_argument("--pretrained", type=str, default="True",
                        help="Whether pretrained weights are available") # new arg added
